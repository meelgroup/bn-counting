...
def split_safe_unsafe(expression: expr) -> Tuple[List[expr], List[expr]]:
    """Split an Or/And into safe and unsafe parts.

    An unsafe part contains a literal and its negation.
    """
    # is_conj_literal = True
    # leaves_list = []
    # for child in expression.xs:
    #     leaves_list.append(leaves(child))
    #     if isinstance(child, Literal) == False:
    #         is_conj_literal = False

    # if is_conj_literal == True:
    #     return (leaves_list, [])
