...
def solve_asp(asp_filename: str, max_output: int, time_limit: int, method: str, computation: str) -> str:
...
def get_asp_output(
    ...
    if not debug and not write_asp_file:
        os.unlink(tmpname)
    return solutions
...
def compute_trap_spaces(
    ...
    if method == "sat":
        ...
        solutions_output = get_asp_output(
            petri_net, max_output, time_limit, computation, method, debug, nprocs, write_asp_file
        )
        if debug:
