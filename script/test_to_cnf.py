...
# remaining time 
remaining_time = 5000 - (time.time() - start_time)
print("#SAT timeout: {}".format(remaining_time))

assert(remaining_time > 0)
cmd = "timeout {} ./ganak -v 0 --maxcache 4000 {}".format(int(remaining_time), sys.argv[1] + ".cnf")
...
for line in out.splitlines():
    if line.startswith("c s exact arb int"):
        l = line.strip().split()
        count = int(l[-1])
        print("#SAT (GANAK) count: {0}".format(count))
